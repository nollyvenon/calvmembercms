...
?>
		...
		$pageprev = $page -1;
		$page1 = $page + 1;
                                         $qr = $db->select('mailtrash',array('*'),"","order BY trash_id DESC LIMIT $limitvalue, $limit")->results();
                                         $q = $db->count('mailtrash',"","order BY trash_id");

?>
                                                    <span class='pull-right mail_count_nav text-muted'><strong>1</strong> to <strong>20</strong> of <?=$trashcnt;?></span>
											...
											if ($curdate == date('Y-m-d',strtotime($row['timestamp']))){//same day
												...
												$mailtime  = date('d M',strtotime($row['timestamp']));														
                                                        ...
                                                        <tr class="unread" id="mail_msg_1">
                                                            ...
                                                            <td class=""><a href="mail-view.php?sid=<?= $row['trash_id'];?>"><?= $row['receiver'];?></a></td>
                                                            <?php    if ($Mailgroup ==""){   ?>
                                                        <td class=""><a href="mail-view.php?sid=<?= $row['trash_id'];?>&t=trash"><span class="msgtext"><?= limit_text($row['message'],10);?></span></a></td>
                                                        <?php }else{ 
...
?>
                                                        <td class=""><a href="mail-view.php?sid=<?= $row['trash_id'];?>&t=trash"><span class="label label-primary"><?= $group_name;?></span>&nbsp;<span class="msgtext"><?= limit_text($row['message'],10);?></span></a></td>
                                                        <?php }  ?>
