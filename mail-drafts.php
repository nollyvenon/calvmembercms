...
?>
		...
		$pageprev = $page -1;
		$page1 = $page + 1;
                                         $qr = $db->select('maildrafts',array('*'),"","order BY draft_id DESC LIMIT $limitvalue, $limit")->results();
                                         $q = $db->count('maildrafts',"","order BY draft_id");
                                        ...
                                        if ($curdate == date('Y-m-d',strtotime($row['timestamp']))){//same day
                                            ...
                                            $mailtime  = date('d M',strtotime($row['timestamp']));														
                                                    ...
                                                    <tr class="unread" id="mail_msg_1">
                                                        ...
                                                        <td class=""><a href="mail-view.php?sid=<?= $row['draft_id'];?>"><?= $row['receiver'];?></a></td>
                                                    <?php    if ($Mailgroup ==""){   ?>
                                                        <td class=""><a href="mail-view.php?sid=<?= $row['draft_id'];?>&t=draft"><span class="msgtext"><?= limit_text($row['message'],10);?></span></a></td>
                                                        <?php }else{ 
...
?>
                                                        <td class=""><a href="mail-view.php?sid=<?= $row['draft_id'];?>&t=draft"><span class="label label-primary"><?= $group_name;?></span>&nbsp;<span class="msgtext"><?= limit_text($row['message'],10);?></span></a></td>
                                                        <?php }  ?>
