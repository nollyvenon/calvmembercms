...
?>
		...
		$pageprev = $page -1;
		$page1 = $page + 1;
                                            $qr = $db->select('smssent',array('*'),"","order BY sent_id DESC LIMIT $limitvalue, $limit")->results();
                                            $q = $db->count('smssent',"","order BY sent_id");
											...
											if ($curdate == date('Y-m-d',strtotime($row['timestamp']))){//same day
												...
												$mailtime  = date('d M',strtotime($row['timestamp']));														
                                                        ...
                                                        <tr class="unread" id="mail_msg_1">
                                                            ...
                                                            <td class="open-view"><a href="sms-view.php?sid=<?= $row['sent_id'];?>"><?= $row['receiver'];?></a></td>
									<?php    if ($Mailgroup ==""){   ?>
                                                        <td class=""><a href="sms-view.php?sid=<?= $row['sent_id'];?>&t=sent"><span class="msgtext"><?= limit_text($row['message'],10);?></span></a></td>
                                                        <?php }else{ 
...
?>
                                                        <td class=""><a href="sms-view.php?sid=<?= $row['sent_id'];?>&t=sent"><span class="label label-primary"><?= $group_name;?></span>&nbsp;<span class="msgtext"><?= limit_text($row['message'],10);?></span></a></td>
