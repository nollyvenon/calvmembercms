...
?>
		...
		$pageprev = $page -1;
		$page1 = $page + 1;
                                         $qr = $db->select('smsdrafts',array('*'),"","order BY draft_id DESC LIMIT $limitvalue, $limit")->results();
                                         $q = $db->count('smsdrafts',"","order BY draft_id");

?>
                                                    <span class='pull-right mail_count_nav text-muted'><strong>1</strong> to <strong>20</strong> of <?=$draftcnt;?></span>
                                        ...
                                        if ($curdate == date('Y-m-d',strtotime($row['timestamp']))){//same day
                                            ...
                                            $mailtime  = date('d M',strtotime($row['timestamp']));														
                                                    ...
                                                    <tr class="unread" id="mail_msg_1">
                                                        ...
                                                        <td class=""><a href="sms-view.php?sid=<?= $row['draft_id'];?>"><?= $row['receiver'];?></a></td>
                                                    <?php    if ($Mailgroup ==""){   ?>
                                                        <td class=""><a href="sms-view.php?sid=<?= $row['draft_id'];?>&t=draft"><span class="msgtext"><?= limit_text($row['message'],10);?></span></a></td>
                                                        <?php }else{ 
...
?>
                                                        <td class=""><a href="sms-view.php?sid=<?= $row['draft_id'];?>&t=draft"><span class="label label-primary"><?= $group_name;?></span>&nbsp;<span class="msgtext"><?= limit_text($row['message'],10);?></span></a></td>
                                                        <?php }  ?>
                                                        <td class="open-view"><span class="msg_time"><a href="sms-view.php?sid=<?= $row['draft_id'];?>&t=draft"><?= $mailtime;?></span></a></td>
                                                    </tr>
                                                    <?php } 
                                        }?>
                                                </table>
