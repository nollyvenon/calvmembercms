...
?>
		...
		$pageprev = $page -1;
		$page1 = $page + 1;
                                            $qr = $db->select('mailsent',array('*'),"","order BY sent_id DESC LIMIT $limitvalue, $limit")->results();
                                            $q = $db->count('mailsent',"","order BY sent_id");

?>
                                                    <span class='pull-right mail_count_nav text-muted'><strong>1</strong> to <strong>20</strong> of <?=$sentcnt;?></span>
											...
											if ($curdate == date('Y-m-d',strtotime($row['timestamp']))){//same day
												...
												$mailtime  = date('d M',strtotime($row['timestamp']));														
                                                        ...
                                                        <tr class="unread" id="mail_msg_1">
                                                            ...
                                                            <td class="open-view"><a href="mail-view.php?sid=<?= $row['sent_id'];?>"><?= $row['receiver'];?></a></td>
									<?php    if ($Mailgroup ==""){   ?>
                                                        <td class=""><a href="mail-view.php?sid=<?= $row['sent_id'];?>&t=sent"><span class="msgtext"><?= limit_text($row['message'],10);?></span></a></td>
                                                        <?php }else{ 
...
?>
                                                        <td class=""><a href="mail-view.php?sid=<?= $row['sent_id'];?>&t=sent"><span class="label label-primary"><?= $group_name;?></span>&nbsp;<span class="msgtext"><?= limit_text($row['message'],10);?></span></a></td>
                                                        <?php }  ?>                                                            <td class="open-view"><span class="msg_time"><?= $mailtime;?></span></td>
                                                        </tr>
                                                        <?php } 
											}?>
                                                    </table>
